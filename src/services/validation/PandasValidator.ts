...
import { Question, QueryResult, ValidationResult, isPandasQuestion } from '@/types';
...
class PandasValidator implements IValidator {
  async validate(question: Question, userCode: string): Promise<ValidationResult> {
    if (!isPandasQuestion(question)) {
      return {
        ...
        totalDatasets: 0,
        passedDatasets: 0,
        error: 'PandasValidator can only validate Pandas questions',
      };
    }
    ...
    try {
      for (const dataset of datasets) {
        for (const table of dataset) {
          await pandasRuntime.createDataFrame(table.name, table.data);
        }

        const userResult = await pandasRuntime.executeCode(userCode);
        if (userResult.error) {
          return {
            passed: false,
            totalDatasets,
        ...
        const expectedResult = await pandasRuntime.executeCode(
...
export const pandasValidator = new PandasValidator();
